...
public class Producto {
	...
	public void ConsulEditarProductos(int cod) {
		Conexion con = new Conexion();
		ResultSet rs = null;
		String sql = "SELECT id_pr, id_cat, nombre_pr, precio_pr, cantidad_pr FROM public.tb_producto where id_pr =" + cod + ";";
		try {
			rs = con.Consulta(sql);
			while(rs.next()) {
	...
	//OFERTAS
	//Metodo para mostrar las opciones para ofertar
	public String consultar_ofertasProducto() {
		String sql = "SELECT * FROM tb_producto ORDER BY id_pr";
		Conexion con = new Conexion();
		String tabla = "<table border=2><th>ID</th><th>Producto</th><th>Cantidad</th><th>Precio</th>";
		ResultSet rs = null;
		...
		try {
			while (rs.next()) {
				...
				+ "<td>" + rs.getInt(4) + "</td>"
				+ "<td>" + rs.getDouble(5) + "</td>"
				+ "<td> <a href=ofertarProducto.jsp?cod=" + rs.getInt(1) + "><pre style=\"text-align: center\">Ofertar</pre></a></td>"
				+ "<td> <a href=no_ofertar.jsp?cod=" + rs.getInt(1) + "><pre style=\"textalign: center\">Reestablecer</pre></a></td>"
				+ "</td></tr>";
			}
		} catch (SQLException e) {
			// TODO Auto-generated catch block
	...
	//Metodo para ofertar
	public boolean oferta_Producto(int cod) {
		boolean f = false;
		Conexion con = new Conexion();
		String sql = "UPDATE public.tb_producto	SET oferta_pr=precio_pr - (precio_pr*0.6), estado_pr='En oferta' WHERE id_pr='"+cod+"';";
		try {
			con.Ejecutar(sql);
			f = true;
		}catch (Exception e) {
	...
	public boolean no_oferta(int cod) {
		boolean f = false;
		Conexion con = new Conexion();
		String sql = "UPDATE public.tb_producto	SET oferta_pr="+0+", estado_pr='Normal' WHERE id_pr='"+cod+"';";
		try {
			con.Ejecutar(sql);
			f = true;
		}catch (Exception e) {
	...
	public String ofertas_detalle() {
		String sql = "SELECT * FROM tb_producto WHERE estado_pr='En oferta'ORDER BY id_pr";
		Conexion con = new Conexion();
		String tabla = "<table border=2><th>ID</th><th>Producto</th><th>Cantidad</th><th>Precio Original</th><th>Precio en oferta</th><th>Estado</th>";
		ResultSet rs = null;
		rs = con.Consulta(sql);
		try {
			while (rs.next()) {
